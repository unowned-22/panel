      ...
      >
        <div className="space-y-5">
          <p className="text-center text-sm text-neutral-400">
            {t('cover.preview.hint')}
          </p>

          <div className="mx-auto flex w-fit rounded-lg bg-neutral-800 p-1">
            {(["desktop", "mobile"] as const).map((tabKey) => (
                <button
                    key={tabKey}
                    onClick={() => setTab(tabKey)}
                    className={`rounded-md px-6 py-1.5 text-sm font-medium capitalize transition-colors ${
                        tab === tabKey ? "bg-neutral-600 text-white" : "text-neutral-400 hover:text-white"
                    }`}
                >
                  {tabKey === "desktop" ? t('cover.preview.tab.desktop') : t('cover.preview.tab.mobile')}
                </button>
            ))}
          </div>
